...
b) Trigger_2: Khi cập nhật giá trị của cột NgayKetThucCungCap,
 ...
 from inserted 
 if (@Day >0 and  @Day <4)
 print 'du lieu cho phep cap nhat'
 else
 begin
 print 'khoang thoi gian dang ki cung cap phuong tien tinh tu ngay bat dau cung cap den het ngay ket thuc cung cap phai nho hon 4 nam'
 end
...
as
RETURN 
select mp.MaMP,DonGia,sum (SoLuongXeDangKi) AS TongSoLuongXeDangKi
from DANGKICUNGCAP AS DKCC Join MUCPHI as mp
on DKCC.MaMP=mp.MaMP
where mp.DonGia=@MucPhi
GROUP BY mp.DonGia,mp.MaMP
...
create function func2(@MaNCC varchar(10))
returns table
as
return
select NCC.MaNhaCC,NCC.TenNhaCC,mAX(DATEDIFF(day,NgayBatDauCungCap,NgayKetThucCungCap))as Thoigiandainhat
from NHACUNGCAP AS NCC join DANGKICUNGCAP AS DKCC
ON NCC.MaNhaCC=DKCC.MaNhaCC
WHERE NCC.MaNhaCC=@MaNCC
GROUP BY NCC.MaNhaCC,NCC.TenNhaCC
