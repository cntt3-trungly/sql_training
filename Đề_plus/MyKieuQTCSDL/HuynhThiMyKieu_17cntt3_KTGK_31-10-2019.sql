...
--cau4 Liet ke nhung khach hang co dia chi la lien chiu ma co sdt bat dau là 090 và....
select * from KHACHHANG
where (DiaChiKH='Lien Chieu' and SoDienThoaiKH like '%090%') or (DiaChiKH='Thanh Khe' and SoDienThoaiKH like '%091%')
--cau5 liet ke nhung khach hang co ho trong ho ten la bui
select * from KHACHHANG
where HoTenKH like'%Bui%'
--cau6 Liet ke thong tin cua toan bo cac cay canh yeu cau sap xep theo mo ta va giam dan theo don gia
select * from CAYCANH
ORDER BY DonGiaChoThue desc
...
--cau9 liet ke .... cua tat ca cac hop dongco trag thai dang cho thue
...
--CAU10 liet ke.... cua tat ca cac hop dong voi yeu cau nhung hoat dong nao chua co mot chi tiet hop dong nao thi cung phai liet ke nhung thong tin do ra
select ct.MaHopDong,ct.MaCayCanh,HD.MaKH,ct.SoLuong,HD.TrangThaiHopDong
from CHITIETHOPDONG AS ct LEFT JOIN HOPDONGCHOTHUE AS HD
ON  ct.MaHopDong=HD.MaHopDong
--CAU11 liet ke thong tin cua khach hang co dia chi hai chau da tung thue cay canh thuoc loai thue cay canh co mo ta la chung o phong khach 
...
select kh.MaKH, HoTenKH, DiaChiKH, SoDienThoaiKH
from KHACHHANG kh,HOPDONGCHOTHUE
except
select kh.MaKH, HoTenKH, DiaChiKH, SoDienThoaiKH
from KHACHHANG kh join HOPDONGCHOTHUE hd on kh.MaKH = hd.MaKH
--c2
...
--Cau 13 liet ke thong tin cua câc khach hang da tung thue laoi cay canh duoc mo ta la chung o phong khach va 
...
select distinct kh.MaKH, HoTenKH, DiaChiKH, SoDienThoaiKH,cthd.NgayBatDauChoThue,cthd.NgayKetThucChoThue
from KHACHHANG kh join HOPDONGCHOTHUE hd on kh.MaKH = hd.MaKH
	join CHITIETHOPDONG cthd on hd.MaHopDong = cthd.MaHopDong
	where hd.MaHopDong in (select MaHopDong
					FROM CHITIETHOPDONG
					where Year(NgayBatDauChoThue)=2016
                     and hd.MaHopDong not in (select MaHopDong
					 from CHITIETHOPDONG where Year(NgayBatDauChoThue)=2017))
--c2
select *
from KHACHHANG
...
--Cau 16 dem tong so khach hang da tung thue cay canh trong nam 2016 voi yeu cau chi thuc hien voi nhung k.hang da tung thue it nhat 2 lan va0 nam 2016
...
--kq duoc sap xep giam dna theo makh
...
--cau 18:cap nhap tramg thai hop dong la bi huy trong bang Hop dong cho thue
update HOPDONGCHOTHUE
set TrangThaiHopDong = 'Bi huy'
where TrangThaiHopDong like 'Chua bat dau' and SoTienDatCoc = '0'

--cau 19: cap  nhat ngayketthuc trong bang cthd thanh null cho nhung cay canh da tung duoc thue tu 2 lan tro len
...
where MaLoaiDV not in (
	...
	drop database Quanlicaycanh


	select distinct kh.HoTenKH,kh.DiaChiKH
	from KHACHHANG as kh join HOPDONGCHOTHUE AS hdct
	on kh.MaKH=hdct.MaKH join CHITIETHOPDONG AS CTHD
	ON hdct.MaHopDong=CTHD.MaHopDong
	join LOAIDICHVU AS LDV on  CTHD.MaLoaiDV=LDV.MaLoaiDV
	WHERE LDV.MoTaLoaiDV='Chung Tet'
	

	select *
from LOAICAYCANH as lcc
where lcc.MaLoaiCC not in (
	select MaLoaiCC
	from CAYCANH as cc join CHITIETHOPDONG as cthd on cc.MaCayCanh=cthd.MaCayCanh
	join HOPDONGCHOTHUE  as hdct on hdct.MaHopDong=cthd.MaHopDong
	)
