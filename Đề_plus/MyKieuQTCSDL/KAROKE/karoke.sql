...
CREATE TABLE ChiTiet_SuDungDV(
 ...
 --câu3 Liệt kê những phòng karaoke chứa được số lượng tối đa dưới 20 khách 
 select *from Phong
 where SoKhachToiDa<20
 --câu4 Liệt kê những dịch vụ karoke với đơn vị tính là chai và đơn giá là 20000 hoạc đơn vị tính là lon và đơn giá là 30000
 select*from DichVu
 where (DonViTinh='Chai' and DonGia<20000)or(DonViTinh='Lon' and DonGia>30000)
 --câu5 Liệt những phòng karoke với mã phòng bắt đầu bằng VIP
 select *from Phong
 where MaPhong like '%VIP%'
 --câu6 Liệt kê thông tin của toàn bộ các dịch vụ, yêu cầu sắp xếp giảm dần theo đơn giá
 select *from DichVu
 order by DonGia desc
 --câu7 Đếm số hóa đơn có trạng thái là "Chưa thanh toán" và có thời gian bắt đầu sử
--dụng nằm trong ngày hiện tại
 select count(*) from HoaDon
 where TrangThaiHD like 'Chua thanh toan'and ThoiGianBatDauSD=GETDATE()
 --câu8 Liệt kê địa chỉ của toàn bộ các khách hàng với yêu cầu mỗi địa chỉ được liệt kê một
...
--toán" 
 ...
 --câu 10Liệt kê MaKH, TenKH, DiaChi, MaHD, TrangThaiHD của tất cả các hóa đơn với
...
--những khách hàng đó ra
	...
	FROM KhachHang KH LEFT JOIN HoaDon HD
		ON KH.MaKH=HD.MaKH
 --câu11 Liệt kê thông tin của các khách hàng đã từng sử dụng dịch vụ "Trái cây" hoặc từng
...
--câu12 Liệt kê thông tin của các khách hàng chưa từng sử dụng dịch vụ hát karaoke lần nào
--cả
...
select kh.MaKH,KH.TenKH
from KhachHang AS kh
WHERE  NOT EXISTS (SELECT MaKH FROM HoaDon AS HD WHERE kh.MaKH =HD.MaKH)
--câu 13Liệt kê thông tin của các khách hàng đã từng sử dụng dịch vụ hát karaoke và chưa
...
--câu14Liệt kê thông tin của những khách hàng đã từng hát karaoke vào năm "2014" nhưng
...
--Câu16 Đếm tổng số lượng loại dịch vụ đã được sử dụng trong năm 2014 với yêu cầu chỉ
...
--Câu17 Liệt kê MaKH, TenKH, MaSoThue của khách hàng có địa chỉ là "Hải Châu" và
...
--câu18Cập nhật cột TrangThaiHD trong bảng HOADON thành giá trị "Đã hết hạn" đối với
...
--câu19 Cập nhật cột MoTa trong bảng PHONG thành giá trị "Được sử dụng nhiều" cho
